    ...
    <div className="bg-slate-900/90 backdrop-blur border-b border-slate-800/80 px-6">
      <nav className="flex space-x-1 overflow-x-auto py-2">
        {tabs.map((tab) => {
          ...
          const isSpecialPitchDeck = tab.id === 'pitchdeck';
          return (
            <button
              key={tab.id}
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold whitespace-nowrap transition-all ${
                isActive
                  ...
                  : isSpecialPitchDeck
                  ? 'text-purple-300 hover:text-purple-100 hover:bg-purple-950/40 border border-purple-500/30'
                  : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/60 border border-transparent'
              }`}
            >
              <Icon className={`w-3.5 h-3.5 ${isActive ? 'text-orange-400' : (isSpecialPitchDeck ? 'text-purple-300' : 'text-slate-400')}`} />
              <span>{tab.label}</span>
              {tab.badge > 0 && (
